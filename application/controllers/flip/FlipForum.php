...
class FlipForum extends CI_Controller{
    ...
    public function index(){
        ...
        $data['dotcom_user_info'] = $this->global_model->get_data('users', array('id' => $loginId));

        $data['social'] = $this->global_model->get('flip_forum_category', array('sec_id' => '1'));
        $data['allprof'] = $this->global_model->get('flip_forum_category', array('sec_id' => '2'));
        $user_type = $this->session->userdata('user_type');
         $get_current_proff= $data['dotcom_user_info']['parent_profession'];
        $data['dynamicprofession'] = $this->global_model->get('flip_forum_category', array('sec_id' => '3', 'group_id' => $get_current_proff, 'status' => 1));




        $this->load->view('flip/flip_header', $data);
        $this->load->view('flip/forum/board', $data);
    ...
    public function listcat(){
        ...
        $data['postdeatils'] = $this->global_model->get('flip_forum_post', array('cat_id' => $data['getid']));
        $data['catName'] = $this->global_model->get_data('flip_forum_category', array('cat_id' => $data['getid']));
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == true) {
                ...
                $i = 1;
                $bata['total_post'] = $this->global_model->count_row_where('flip_forum_post', array('cat_id' => $countneedid));
                ...
                if ($ref_id = $this->global_model->insert('flip_forum_post', $save))
                {
                    $id = $this->uri->segment('3');
                    if($ref_id = $this->global_model->update('flip_forum_category', $bata, array('cat_id' => $id)))

                    $this->session->set_flashdata('message', 'New Forum Post Created Successfully');
                    $redirect_link = base_url() . 'FlipForum/listcat/'. $postData['hiddencat'];
        ...
        $this->load->view('flip/flip_header', $data);
        $this->load->view('flip/forum/listcatname', $data);
        $this->load->view('flip/flip_footer', $data);
    ...
    public function discuss(){
        ...
        $data['getid']  = $this->uri->segment('3');
        $data['postdeatils'] = $this->global_model->get_data('flip_forum_post', array('post_id' => $data['getid']));
        /// Author
        $data['user_post_info'] = $this->global_model->get_data('flip_users', array('uid' => $data['postdeatils']['author_id']));
        /// Comments retrive
        $data['comments'] = $this->global_model->get('flip_forum_comments', array('post_id' => $data['getid']));
        $data['totalComments'] = $this->global_model->count_row_where('flip_forum_comments', array('post_id' => $data['getid']));

        /// comments author
        $data['getcommentsid'] = $this->global_model->get_data('flip_forum_comments', array('post_id' => $data['getid']));
        $data['user_comments_info'] = $this->global_model->get_data('flip_users', array('uid' => $data['getcommentsid']['user_id']));
        ...
        if($getid == true){
            ...
            $this->global_model->update('flip_forum_post', $viewdata, array('post_id' => $getid));
        }
        //////// PAGE VIEW ++++
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == true) {
                ...
                if ($ref_id = $this->global_model->insert('flip_forum_comments', $save)) {

                    //////// Comment VIEW ++++
                    $gettotalcomments =$postData['totalcomments'];
                    $updatecomment = $gettotalcomments + 1;
                    $bata['reply'] = $updatecomment;
                    $this->global_model->update('flip_forum_post', $bata, array('post_id' => $postData['postid']));
                    //////// Comment VIEW ++++



                    $this->session->set_flashdata('message', 'New Forum Post Created Successfully');
                    $redirect_link = base_url() . 'FlipForum/discuss/'. $postData['postid'];
                    redirect($redirect_link);
        ...
        $this->load->view('flip/flip_header', $data);
        $this->load->view('flip/forum/discusstopic', $data);
        $this->load->view('flip/flip_footer', $data);
    ...
    public function addcat()
    {
        ...
        $this->load->view('flip/flip_header', $data);
        $this->load->view('flip/forum/catadd', $data);
        $this->load->view('flip/flip_footer', $data);
...
//// SHW ALL PRODUCT
    public function allmypostlist()
    {
        ...
        $data = array();
        $data['page_title'] = 'Forum All My Post';
        $data['error'] = '';
        $loginId = $this->session->userdata('login_id');
        $data['user_info'] = $this->global_model->get_data('flip_users', array('uid' => $loginId));

        $data['allmypostlist'] = $this->global_model->get('flip_forum_post', array('author_id' => $loginId));



        $this->load->view('flip/flip_header', $data);
        $this->load->view('flip/forum/postlist', $data);
        $this->load->view('flip/flip_footer', $data);
    ...
    public function allmycomments()
    {
        ...
        $data = array();
        $data['page_title'] = 'Forum All My Comments';
        $data['error'] = '';
        $loginId = $this->session->userdata('login_id');
        $data['user_info'] = $this->global_model->get_data('flip_users', array('uid' => $loginId));

        $data['allmycomments'] = $this->global_model->get('flip_forum_comments', array('user_id' => $loginId));




        $this->load->view('flip/flip_header', $data);
        $this->load->view('flip/forum/allmycomments', $data);
        $this->load->view('flip/flip_footer', $data);
    ...
    public function editPost($id){
        ...
        if ($this->input->post()) {
            ...
            if ($this->global_model->update('flip_forum_post', $save, array('post_id' => $id))){
                $this->session->set_flashdata('message', 'Post updated successfully.');
                //redirect('profile/profile');
            }
        }

        $data['editMyPost'] = $this->global_model->get_data('flip_forum_post', array('post_id' => $id));
        $data['login_id'] = $loginId;



        $this->load->view('flip/flip_header', $data);
        $this->load->view('flip/forum/editPost', $data);
        $this->load->view('flip/flip_footer', $data);
    ...
    public function editComment($id){
        ...
        if ($this->input->post()) {
            ...
            if ($this->global_model->update('flip_forum_comments', $save, array('comment_id' => $id))){
                $this->session->set_flashdata('message', 'Comment updated successfully.');
                //redirect('profile/profile');
            }
        }

        $data['editMyComment'] = $this->global_model->get_data('flip_forum_comments', array('comment_id' => $id));
        $data['login_id'] = $loginId;



        $this->load->view('flip/flip_header', $data);
        $this->load->view('flip/forum/editComment', $data);
        $this->load->view('flip/flip_footer', $data);
